    ...
    public static Task<(
        ...
        CancellationToken cancellationToken = default) => Task.Run(() =>
        {
            var languageNamePrefixes = new[] {
                preferredLanguageName.ToLowerInvariant(),
                "en",
                string.Empty,
            };
            ...
            try {
                foreach (var familyIndex in Enumerable.Range(0, collection.FontFamilyCount)) {
                    ...
                    foreach (var languageNamePrefix in languageNamePrefixes) {
                        var localeNameIndex = tempLanguageNames
                            .Select((x, i) => (x, 1 + i))
                            .FirstOrDefault(x => x.x.StartsWith(languageNamePrefix))
                            .Item2 - 1;
                    ...
                    foreach (var fontIndex in Enumerable.Range(0, family.FontCount)) {
                        ...
                        tempVariants.Add(FontIdent.FromSystem(name, font.Weight, font.Stretch, font.Style));
                    }
